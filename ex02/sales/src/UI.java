...
public class UI {
	...
	static Helpers[] operations = { (store) -> printByWeek("| Total sales per week ::", store.totalSalesPerWeek()),
			...
			(store) -> Arrays.stream(operationsRef).limit(6).forEach(s -> s.invoke(store)), (store) -> {
				System.out.println("| Exited to main menu");
				operationsLoop = false;
			} };

	// only allows the user to input integers and won't give up until an integer is
	// inputed
	...
	public void loop() {
		...
		while (true) {
			...
			selectedStoreIndex = askForIntBetween(
					String.format("Select a store by number (1 through %d) or enter %d to terminate the program",
